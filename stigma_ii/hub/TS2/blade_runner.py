...
data = data.dropna(axis=1).sample(n=100, axis=1)
target = data.columns.values[99]
...
ax[0, 0].plot(range(yy_train.shape[0]), yy_train.flatten() - y_hat_train.flatten(), color='navy')
ax[1, 0].plot(range(yy_test.shape[0]), yy_test.flatten() - y_hat_test.flatten(), color='orange')
ax[0, 1].hist(yy_train.flatten() - y_hat_train.flatten(), color='navy', bins=100, density=True)
ax[1, 1].hist(yy_test.flatten() - y_hat_test.flatten(), color='orange', bins=100, density=True)

pyplot.plot(range(yy_train.shape[0]), yy_train, 'black', range(yy_train.shape[0]), y_hat_train, 'blue')
pyplot.plot(range(yy_test.shape[0]), yy_test, 'black', range(yy_test.shape[0]), y_hat_test, 'blue')
