...
class Test(nn.Module):
    ...
    def fit(self, X_train, X_val, Y_train, Y_val, Y_train_base, Y_val_base):
        ...
        for i in range(self._epochs):
            ...
            for phase in ['train', 'validate']:
                ...
                if phase == 'train':
                    y_pred = (self(X_train) + 1) * Y_train_base
                    single_loss = self.loss_function(y_pred, Y_train)
                else:
                    y_pred = (self(X_val) + 1) * Y_val_base
    ...
    def predict(self, X, Y_base):

        output = self(X)
        result = output.detach().numpy()

        result = (result + 1) * Y_base
...
class NN(nn.Module):
    ...
    def fit(self, X_train, y_train, y_train_base, X_val, y_val, y_val_base, loss_function):
    ...
    def predict(self, X_test, y_base):
...
class WrappedNN:
    ...
    def fit(self, X_train, Y_train, Y_train_base, X_val, Y_val, Y_val_base):
        self.model.fit(X_train=X_train, y_train=Y_train, y_train_base=Y_train_base,
                       X_val=X_val, y_val=Y_val, y_val_base=Y_val_base,
    ...
    def predict(self, X, Y_base):
        return self.model.predict(X, y_base=Y_base)
